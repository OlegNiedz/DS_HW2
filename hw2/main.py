...
def db_document_update(db_collection, old_name,kwargs):
    ...
    if kwargs["name"] and len(list(db_collection.find({"name": kwargs["name"]}))) == 0:
        try:
            result = db_collection.update_one({"name": kwargs["name"]},
            {
                "$set": kwargs
            })
            if result:
...
def main():
    ...
    if db_connection:
        ...
        def promt(msg_promt: str, variants: tuple = (), msg_err="", ignore_caps=False, allow_empty=False):
            while True:
        ...
        while True:
            ...
            match command:
                ...
                case "change":
                    ...
                    if cat:
                        name = promt(f"name ={cat.name}; Enter new name (empty without change): ",allow_empty=True)
                        age = promt(f"age ={cat.age}; Enter new age (from 1 to 25) (empty without change): ",tuple(range(1, 25)),
                            allow_empty=True)
                        features =[]
                        feature = promt(
                        ...
                        if name and age:
                            db_document_update(
                                db_collection=db_collection,
                                kwargs=build_kwargs(name, age, features),old_name=cat.name
                            )

                case "show":
                    db_document_read(
                        db_collection=db_collection,
                        name=promt("Enter name (empty for all cats): ",allow_empty=True)
                    )
